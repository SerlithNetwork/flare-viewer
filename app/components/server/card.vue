...
const knownKeys = [
  ...
  "flare:proxy:playercount",
  "flare:proxy:servercount",
];
...
categories.set("builtin:stat:memory_used", {
  value: { name: "Memory Used", color: "#FFB46E" },
});
categories.set("builtin:stat:memory_total", {
  value: { name: "Memory Allocated", color: "#FF996E" },
});
categories.set("builtin:stat:cpu_process", {
  value: { name: "CPU Process", color: "#A579F7" },
});
categories.set("builtin:stat:cpu", {
  value: { name: "CPU System", color: "#7B38E8" },
});
categories.set("airplane:world:playercount", {
  value: { name: "Players", color: "#FCD4FF" },
});
categories.set("airplane:world:entitycount", {
  value: { name: "Entities", color: "#FCC7FF" },
});
categories.set("airplane:world:chunkcount", {
  value: { name: "Chunks", color: "#FF8787" },
});
categories.set("airplane:world:blockentitycount", {
  value: { name: "Block Entities", color: "#FCBDFF" },
});
categories.set("builtin:gc:minor", {
  value: { name: "Minor GC Pause", color: "#5476FF" },
});
categories.set("builtin:gc:generic", {
  value: { name: "Generic GC Pause", color: "#2954FF" },
});
categories.set("builtin:gc:major", {
  value: { name: "Major GC Pause", color: "#082FC9" },
});
categories.set("builtin:thread:startedcount", {
  value: { name: "Threads Started", color: "#4DFFC0" },
});
categories.set("builtin:thread:peakcount", {
...
categories.set("builtin:thread:daemoncount", {
  value: { name: "Daemon Threads", color: "#C7FFCC" },
});
...
const unknownMetrics = computed(() =>
  summary.value.metrics
    .entries()
    .filter((entry) => !knownKeys.includes(entry[0]))
    .toArray(),
);
const unknownEvents = computed(() =>
...
<template>
  <div class="flex flex-col items-center min-w-fit m-8 max-w-sm gap-6">
    ...
    <UCard
      v-if="unknownMetrics.length + unknownEvents.length > 0"
    ...
    >
      <div class="flex flex-col items-center w-full gap-4">
        ...
        <div class="flex flex-wrap justify-around w-full gap-4">
          <AreaChart
            v-for="metric in unknownMetrics"
            class="w-[70vh]"
            :data="metric[1]"
